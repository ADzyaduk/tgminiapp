      ...
      <div v-if="!isManualTime">
        ...
        <div class="flex flex-wrap gap-3 mb-6">
          <UButton
            ...
            :key="hour"
            :disabled="isBooked(hour) || isPastHour(hour)"
            :variant="variantFor(hour)"
            class="min-w-[80px]"
        ...
        <div class="grid grid-cols-1 md:grid-cols-2 gap-4 mb-4">
          ...
          <UFormField label="Время окончания">
            <UInput 
              ...
              type="time"
              :min="manualStartTime || minTimeInput"
              class="w-full"
            />
          </UFormField>
        </div>
...
function variantFor(hour: number) {
  if (isPastHour(hour)) {
    return 'soft'
  }
  return selectedSlot.value === hour
    ? 'solid'
    : bookedSlots.value.includes(hour)
      ? 'outline'
      : 'outline'
}
